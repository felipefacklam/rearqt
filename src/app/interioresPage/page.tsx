  ...
  useEffect(() => {
    const fetchProjetos = async () => {
      try {
        ...
        const interiores = data.filter(projeto => projeto.type === "Interiores");
        setProjetos(interiores);
      } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-screen w-screen">
      ...
      <div className="flex flex-col flex-grow items-center pb-20 bg-zinc-200">
        <h1 className="p-10 text-3xl text-gold-primary drop-shadow-sm">Projetos de Interiores</h1>
        {projetos ? (
          <div className="grid grid-cols-2 gap-4 p-5">
            {projetos.map((projeto, index) => (
              <div key={projeto._id} onClick={() => handleProjetoClick(projeto._id)} className="cursor-pointer">
                <h2 className="text-center font-bold">{projeto.title}</h2>
                <Image
